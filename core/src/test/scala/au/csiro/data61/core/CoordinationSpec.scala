...
class CoordinationSpec  extends FunSuite with JsonFormats with BeforeAndAfterEach with LazyLogging {
  ...
  val businessDs = Paths.get(sampleDsDir, datasetMap("businessInfo").toString, "businessinfo.csv").toFile
  val businessSsd = Paths.get(ssdDir, "businessInfo.ssd").toFile

  val citiesDs = Paths.get(sampleDsDir, datasetMap("getCities").toString, "getcities.csv").toFile
