...
class SparkTestSpec extends FunSuite {
  ...
  def trainRandomForest(sc: SparkContext, sqlContext: SQLContext): RandomForestClassificationModel = {
  ...
  test("one worker vs two workers") {
    ...
    val oneCoreModel = trainRandomForest(sc, sqlContext)
    sc.stop()

    val (sc2, sqlContext2) = setUpSpark("2")
    val twoCoreModel = trainRandomForest(sc2, sqlContext2)
    sc2.stop()
