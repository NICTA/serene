...
//    res.asJava.asInstanceOf[util.List[SemanticType]]
  ...
  private def getAlignNodeMappings(karmaSSD: KarmaSSD): util.HashMap[Node,Node] = {
    ...
    val internalMap = suggestModelLearningGraph.asInstanceOf[ModelLearningGraphCompact]
      .getInternalNodeMapping(karmaSSD)
    val colMap = suggestModelLearningGraph.asInstanceOf[ModelLearningGraphCompact]
