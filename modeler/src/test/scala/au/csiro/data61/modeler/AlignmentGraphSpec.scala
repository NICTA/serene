...
import au.csiro.data61.core.types.SemanticSourceDesc
...
class AlignmentGraphSpec extends FunSuite with ModellerJsonFormats with BeforeAndAfterEach with LazyLogging {
  ...
  test("Constructing initial alignment graph using Karma") {
    ...
    // our alignment
    val alignment = karmaTrain.constructInitialAlignment()
  ...
  test("Duplicating SSD to the alignment graph") {
    ...
    val newSSD = SSDStorage.get(0) match {
