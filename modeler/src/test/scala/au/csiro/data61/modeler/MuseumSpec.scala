...
/**
  * Some tests for converting museum files to our format
  */
...
//    assert(conv.semanticModel.get.getHelperLinks.map(_.prefix).distinct.size > 1)
  ...
  test("Soccer dataset models conversions"){
    val jsonList: Array[String] = Paths.get(karmaDir, "soccer")
      .toFile.listFiles
      .filter(_.toString.endsWith(Params.MODEL_MAIN_FILE_EXT))
      .map(_.getAbsolutePath)
    ...
    assert(semanticModels.length === 12)

    // read in ontologies
    val ontologyNames = Paths.get(karmaDir, "soccer", "preloaded-ontologies")
      .toFile.listFiles.map(_.getName).zipWithIndex.map(x => (x._2,x._1)).toMap
    ...
    val path = Paths.get(karmaDir, "soccer","conversion")
    convertedSemModels.foreach(sm => writeToFile(path, sm))
